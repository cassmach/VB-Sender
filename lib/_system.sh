...
system_create_user() {
  print_banner
  printf "${WHITE} 💻 Agora, vamos criar o usuário para deployvbsender...${GRAY_LIGHT}"
  printf "\n\n"

  sleep 2
 
  sudo su - root <<EOF
  useradd -m -p $(openssl passwd $deploy_password) -s /bin/bash -G sudo deployvbsender
  usermod -aG sudo deployvbsender
  mv "${PROJECT_ROOT}"/whaticket.zip /home/deployvbsender/
EOF
...
system_node_install() {
  ...
  sudo -u postgres psql -c "CREATE DATABASE vbsender;"
  exit
EOF
...
system_pm2_install() {
  ...
  sudo su - root <<EOF
  npm install -g pm2
  pm2 startup ubuntu -u deployvbsender
  env PATH=\$PATH:/usr/bin pm2 startup ubuntu -u deployvbsender --hp /home/deployvbsender
EOF

...
frontend_node_dependencies() {
  ...
  cd /home/deployvbsender/whaticket/frontend
  npm install --force
EOF
...
frontend_set_env() {
  ...
  sudo su - deployvbsender << EOF
  cat <<[-]EOF > /home/deployvbsender/whaticket/frontend/.env
...
REACT_APP_NAME_SYSTEM=vbsender
...
move_whaticket_files() {
  ...
  sudo mv /root/whaticket/frontend/vbsender /home/deployvbsender/whaticket/frontend
  sudo mv /root/whaticket/frontend/package.json /home/deployvbsender/whaticket/frontend
  sudo mv /root/whaticket/backend/vbsender /home/deployvbsender/whaticket/backend
  sudo mv /root/whaticket/backend/package.json /home/deployvbsender/whaticket/backend
...
backend_restart_pm2() {
  ...
  sudo su - deployvbsender <<EOF
    cd /home/deployvbsender/whaticket/backend
    pm2 stop all
    sudo rm -rf /root/vbsender
EOF

  sleep 2

  sudo su - <<EOF
    usermod -aG sudo deployvbsender

    grep -q "^deployvbsender ALL=(ALL) NOPASSWD: ALL$" /etc/sudoers || echo "deployvbsender ALL=(ALL) NOPASSWD: ALL" >> /etc/sudoers

    echo "deployvbsender ALL=(ALL) NOPASSWD: ALL" | EDITOR='tee -a' visudo
EOF
